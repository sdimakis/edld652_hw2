...
tbl_data <- crime %>% # change reported_date to just the year
  ...
  dplyr::select(c(DISTRICT_ID, OFFENSE_CATEGORY_ID, YEAR)) %>% # get the offense count for each district/year
  count(OFFENSE_CATEGORY_ID, DISTRICT_ID, YEAR) %>% # group offense count by year
  pivot_wider(names_from = "YEAR",
              values_from = "n") %>% # choose only the offense categories we want for the table
  filter(OFFENSE_CATEGORY_ID == "aggravated-assault" |
           OFFENSE_CATEGORY_ID == "sexual-assault" |
           OFFENSE_CATEGORY_ID == "murder" |
           OFFENSE_CATEGORY_ID == "other-crimes-against-persons") %>% # choose only districts 1,3,5
  filter(DISTRICT_ID == "1" |
           DISTRICT_ID == "3" |
           DISTRICT_ID == "5") %>% # replace all NAs with 0
  replace(is.na(.), 0) %>% # change the names of each offense
  mutate(OFFENSE_CATEGORY_ID = case_when(
    ...
    OFFENSE_CATEGORY_ID == "other-crimes-against-persons" ~ "Other Crimes Against Persons")) %>% # change the names of each district
